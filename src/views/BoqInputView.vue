<template>
    <div class="card mb-2 bg-white text-gray-800 rounded-lg shadow p-2">
        <div class="grid grid-cols-12 gap-1">
                ...
                <Select id="plan" v-model="selectedPlan" :options="plans" filter optionLabel="name" size="small"
                    ...
                    <template #value="slotProps">
                        <div v-if="slotProps.value" class="flex items-center">
                            <div>{{plans.findIndex(h => h.id === slotProps.value.id) + 1}} - {{ slotProps.value.name
                            }}</div>
                        </div>
                        <span v-else>
            ...
            <div class="col-span-12 md:col-span-6">
                ...
                <Select id="house-no" v-model="selectedHouse" :options="houses" filter optionLabel="name" size="small"
                    ...
                    <template #value="slotProps">
                        <div v-if="slotProps.value" class="flex items-center">
                            <div>{{houses.findIndex(h => h.id === slotProps.value.id) + 1}} - {{ slotProps.value.name
                            }}</div>
                        </div>
                        <span v-else>
                            {{ slotProps.placeholder }}
